...
4. `Attachment`/`Attachments`, sequence: Attachments in the picture. It's recommended to use forEach to list all attachments.
...
16. `ExternalLink`, string: A string containing a link to the current post. 

Functions:

The following are functions, simple, logical procedures to do lots of stuff!

1. `ForEach`, function, 1: sequence: A function that loops over a sequence of things. Like attachments and parses a block (Which can be ended with `:End`)

2. `DisplayGenActivity`, function, 0: A very generic function that just displays any Activity or Special object, like Polls or similar. The HTML is generated by Pothole and in most cases works fine with existing themes.

3. `End`, function, 1 (Optional): block: This functions marks the end of block, if no argument is provided then it ends the last-created block. 

4. `HowMany`, function 1: sequence: This function counts the number of items in any given sequence and returns it.

5. `Shorten`, function 1: string (Username): This function takes a username and shortens it.
...
For example, say you want to check if a post has a content warning, well then you can use the following structure:

```
{{:Has = CW}}
<b>Content warning:</b> {{CW}}
{{:End}}
```
...
1. `Has`, function, 1: item: This function checks if an item exists and if it is not empty. If its true then it will parse the block inside of it.

2. `IsUpdated`, function 0: Checks if a post was updated/edited after being written, this is the same as `:Has = UpdatedAt`

3. `IsReply`, function, 0: checks if a post is a reply and if it is then it parses the block inside.

4. `IsExternal`, function, 0: Checks if a post is from an external user/server and if it is then it parses the block inside.
