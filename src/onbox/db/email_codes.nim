...
proc emailCodeExists*(db: DbConn, code: string): bool =
  db.getRow(
    sql"SELECT EXISTS(SELECT 0 FROM email_codes WHERE id = ?);",
    code
  )[0] == "t"

proc emailCodeExistsByUser*(db: DbConn, user: string): bool =
  db.getRow(
    sql"SELECT EXISTS(SELECT 0 FROM email_codes WHERE uid = ?);",
    user
  )[0] == "t"
