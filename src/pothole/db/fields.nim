...
proc fieldExists*(db: DbConn, user, key, value: string): bool =
  ...
  db.getRow(
    sql"SELECT EXISTS(SELECT 0 FROM fields WHERE uid = ? AND key = ? AND value = ?);",
    user, key, value
  )[0] == "t"
...
proc insertField*(db: DbConn, user, key, value: string) =
  ...
  db.exec(
    sql"INSERT INTO fields (uid, key, val) VALUES (?,?,?);",
    user, key, value
  )
...
proc removeField*(db: DbConn, user, key, value: string) =
  ...
  db.exec(
    sql"DELETE FROM fields WHERE uid = ? AND key = ? AND value = ?;",
    user, key, value
  )
...
proc verifyField*(db: DbConn, user, key, value: string, date: DateTime = now().utc) =
  ...
  db.exec(
    sql"UPDATE fields SET verified = true, verified_at = ?, WHERE uid = ? AND key = ? AND value = ?;",
    !$(date), user, key, value
  )
...
proc unverifyField*(db: DbConn, user, key, value: string) =
  ...
  db.exec(
    sql"UPDATE fields SET verified = false WHERE uid = ? AND key = ? AND value = ?;",
    user, key, value
  )